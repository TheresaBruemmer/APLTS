...
def Photons_cone(laser_wavelength,laser_pulse_energy,laser_FWHM_duration,laser_waist,ebunch_gamma_e,ebunch_charge,ebunch_RMS_waist,ebunch_RMS_divergence,ebunch_RMS_length,ebunch_norm_emittance,collimation_angle,angle_steps=50):
    """
    ...
    if type(sigma_e) is np.ndarray:
        ...
        for i,y in enumerate(yy):
            for theta in tt:
                spectrum[i]+=3/(2*sigma_div**2)*(1-2*y*(1-y))*mp.exp(-(theta**2+((1-y)/(gamma_e**2*y)))/(2*s**2))*mp.besseli(0, theta*np.sqrt((1-y)/(gamma_e**2*y))/sigma_div**2)*theta*deltatheta
