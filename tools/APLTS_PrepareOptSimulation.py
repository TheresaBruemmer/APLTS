#Dev Copy of
#APLTS_PrepareOptSimulation.py
"""
Thomson Yield Optimiser within given BW and opening angle
"""
#This file combines APLTS_FindSimData and TS_Optimisation script (the latter does not exist yet, but should be introduced as individual code). 
#ToDo: Make this script a simulation prep tool which uses APLTS_FindSimData and TS_Optimisation

#end of file is prep for simulation, should be separated and in Simulation_tools/Preparation, this yield optimizer should be called
...
Functions
"""
#These are imported from Laser as Laser.a0_calc() or they are used as self. function in the Laser class
...
#These are imported from ThomsonScatteringTools or as method in TS class as TS.Photons_tot and TS.Photons_cone
...
for opt_run in range(0,4):
    ...
    #yield_cone_div_theta exists in TS class and in TS tools
    result = Parallel(n_jobs = num_cores)(delayed(TST.Photons_cone)(llambda,Ep,tau_arr[i],w0_arr[i],gamma_e,Q,sigma_arr_theo,eps_n/gamma_e/sigma_arr_theo,sigma_el,eps_n,theta_c) for i,t in enumerate(tau_arr))
